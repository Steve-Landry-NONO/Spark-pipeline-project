...
1. **Ingestion propre** : lire le CSV brut avec un schéma explicite (`StructType`), typer, nettoyer
   (valeurs manquantes, doublons, valeurs aberrantes), écrire une couche intermédiaire en Parquet.
   Si l'ingestion est sale, toute l'analyse est faussée.
...
5. **Une exploration au-delà du cours** : choisir une piste du menu (section 5), aller plus loin
...
> Tout ce qui dépasse ce socle (deuxième exploration, streaming, MLlib, Delta) est un bonus. Mieux
> vaut un socle solide qu'un bonus bancal posé sur un socle fragile.

---

## 2. Choisir son jeu de données

Deux options, décrites dans `data/sources-open-data.md`. Le taxi et le DVF ont servi de fil rouge
aux TP : le projet se fait sur un jeu neuf, que vous prenez en main vous-mêmes. Choisissez selon
votre appétence.
...
- Nettoyer : doublons (`dropDuplicates`), manquants (`na.drop`/`na.fill`), aberrants (valeurs
  négatives, hors bornes, dates incohérentes).
- Écrire la couche silver en Parquet, partitionnée par une colonne à faible cardinalité (année,
  département, ou catégorie).
...
> que ça tourne, pas en fin de journée. Un chiffre relevé après coup n'est pas reproductible. Visez
> un document dense et lisible : extraits de code, extraits de résultats, captures, phrases courtes.
...
- **AQE et nombre de partitions.** Mesurer l'effet de l'Adaptive Query Execution et du nombre de
  ...
  plan ?
- **Données mal réparties (skew).** Certaines clés concentrent les lignes (quelques départements, ou
  les films les plus notés). Mesurer le déséquilibre des tasks d'un même stage, le corriger, puis
  remesurer.
...
- **Commencez petit.** Un seul fichier (une année ONISR, ou MovieLens small). Faites tourner le
  pipeline de bout en bout sur un petit volume avant d'ajouter des fichiers. Ce qui marche sur
  100 000 lignes marchera sur des millions.
...
- **Gardez du temps pour la rédaction.** Arrêtez le code assez tôt. Un rapport clair sur un pipeline
